...
#include "IRgByPixelisation/videoProcessingCL.hpp"
...
//    size_t *local4PixelMaxWrokGroupSize = (size_t *)jlocal4PixelMaxWrokGroupSize;
    ...
    *p_thresholdEntireImage = (int)jthresholdEntireImage;
    ...
    jfieldID thizthresholdROIFactorJFieldID = env->GetFieldID(klazz, "thresholdROIFactor", "I");
    assert(thizthresholdROIFactorJFieldID != nullptr);
    int *p_thresholdROIFactor = (int *)malloc(sizeof(int));
    jint jthresholdROIFactor=(env)->GetIntField(thiz,thizthresholdROIFactorJFieldID);
    *p_thresholdROIFactor = (int)jthresholdROIFactor;
...
Java_com_example_AndroidOpenCvOpenClNativeExample_NAFFD_processFrameRunJNI(JNIEnv *env,
    ...
    int* p_thresholdROIFactor = (int *)jp_thresholdROIFactor;
